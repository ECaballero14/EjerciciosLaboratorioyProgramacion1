...
void cargarEmpleado(eEmpleado lista[],eSector Sectores[],eEmpSec[], int ts,eEmpSec listaEmpSec[],int tes)
{
    int i;
...
void hardcodearDatosEmpleadosSectores(eEmpSec lista[], int tam)

{
    int i;
    int legajosemp[]= {1,8,9,7,2,4};
    char idSector[][50]= {"1,2,3,1,2,2"};
    int canthoras[50]={"4,2,5,3,6"};

    for(i=0; i<tam; i++)
    {
        lista[i].legajoemp = legajoemp[i];
        lista[i].idSector=idSector[i];
        lista[i].cantidadHoras=cantidadHoras[i];
    }
